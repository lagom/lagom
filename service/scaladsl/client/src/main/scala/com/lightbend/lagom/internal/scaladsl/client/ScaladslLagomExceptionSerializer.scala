...
import com.lightbend.lagom.scaladsl.api.transport.{ ExceptionMessage, PayloadTooLarge, TransportErrorCode, TransportException }

import scala.collection.immutable.Seq

class ScaladslLagomExceptionSerializer(exceptionSerializer: ExceptionSerializer) extends LagomExceptionSerializer {
  override def deserializeWebSocketException(code: Int, requestProtocol: LagomMessageProtocol, bytes: ByteString): Throwable = {
    ...
    exceptionSerializer.deserialize(RawExceptionMessage(errorCode, LagomTransportConverters.convertLagomMessageProtocol(requestProtocol), bytes))
  }
  ...
  override def deserializeHttpException(code: Int, responseProtocol: LagomMessageProtocol, bytes: ByteString): Throwable = {
    ...
    exceptionSerializer.deserialize(RawExceptionMessage(errorCode, LagomTransportConverters.convertLagomMessageProtocol(responseProtocol), bytes))
  }

  override def serialize(exception: Throwable, acceptedProtocols: Seq[LagomMessageProtocol]): LagomRawExceptionMessage = {
    val rawException = exceptionSerializer.serialize(exception, acceptedProtocols.map(LagomTransportConverters.convertLagomMessageProtocol))
  ...
  override def payloadTooLarge(message: String): Exception = PayloadTooLarge(message)
