...
object TaggedOffsetTopicProducer {
  ...
  ): TaggedOffsetTopicProducer[Payload, Event] = new TaggedOffsetTopicProducer(tags, readSideStream)
  ...
  def withoutMetadata[Payload, Event <: AggregateEvent[Event]](
    ...
    (tag, offset) =>
      readSideStream(tag, offset).map {
        case Pair(payload, newOffset) => Pair(Message.create(payload), newOffset)
      }
  )
