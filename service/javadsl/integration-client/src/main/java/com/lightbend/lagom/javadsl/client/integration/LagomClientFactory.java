...
public class LagomClientFactory implements Closeable {
  ...
  public <T> T createDevClient(Class<T> clientInterface, URI serviceLocatorUri) {
    ...
    ServiceLocator serviceLocator =
        new ServiceRegistryServiceLocator(
            circuitBreakersPanel,
            serviceRegistry,
            new ServiceRegistryServiceLocator.ServiceLocatorConfig(serviceLocatorUri),
  ...
  public void close() {
    ...
    closeGracefully(this::terminateActorySystem);
    closeGracefully(() -> eventLoop.shutdownGracefully(0, 10, TimeUnit.SECONDS));
  }

  private void terminateActorySystem() throws InterruptedException, TimeoutException {
    if (managedActorSystem) actorSystem.terminate();
  }
        ...
        serviceLocator -> {
          ...
          TopicFactory kafkaTopicFactory =
              new KafkaTopicFactory(
                  serviceInfo, actorSystem, materializer, actorSystem.dispatcher(), serviceLocator);
