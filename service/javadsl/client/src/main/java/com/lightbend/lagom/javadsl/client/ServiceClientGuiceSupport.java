...
public interface ServiceClientGuiceSupport {
  ...
  /**
   * Binds a client for <code>clientInterface</code> which Guice's Injector makes available so
   * client classes may declare the dependency on their <code>@Inject</code> annotated constructor.
   *
   * <p>Applications that want to consume Lagom services must provide a {@link ServiceInfo}. Using
   * {@link ServiceClientGuiceSupport#bindClient(Class)} requires providing a {@link ServiceInfo}
   * via {@link ServiceClientGuiceSupport#bindServiceInfo(ServiceInfo)}.
   */
  default <T> void bindClient(Class<T> clientInterface) {
    BinderAccessor.binder(this)
        .bind(clientInterface)
        .toProvider(new ServiceClientProvider<T>(clientInterface))
        .in(Singleton.class);
  }

  /**
   * Registers a {@link ServiceInfo} for this application. This step is required to interact with
   * Lagom services.
   *
   * <p>This method must be invoked exactly once.
   *
