 ...
 *
 * <p>Calls to remote services can make use of this facility in order to add circuit breaking
 * capabilities to it.
 */
public interface CircuitBreakersPanel {
  /**
   * Executes {@code body} in the context of the circuit breaker identified by {@code id}. Whether
   * {@code body.get()} is actually invoked is implementation-dependent, but implementations should
   ...
   *     a {@code RuntimeException} to signal failure.
   * @param <T> the result type
   * @return a completion stage yielding either the same result as {@code body.get()}, or failing
