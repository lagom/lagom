 ...
 * A request header.
 *
 * <p>This header may or may not be mapped down onto HTTP. In order to remain agnostic to the
 * underlying protocol, information required by Lagom, such as protocol information, is extracted.
 * It is encouraged that the protocol information always be used in preference to reading the
 * information directly out of headers, since the headers may not contain the necessary protocol
 * information.
 *
 * <p>The headers are however still provided, in case information needs to be extracted out of non
 * standard headers.
 */
public final class RequestHeader extends MessageHeader {
  ...
  public RequestHeader withMethod(Method method) {
    return new RequestHeader(
        method, uri, protocol, acceptedResponseProtocols, principal, headers, lowercaseHeaders);
  }
   ...
   * @return A copy of this request header.
   */
  public RequestHeader withAcceptedResponseProtocols(
      PSequence<MessageProtocol> acceptedResponseProtocols) {
    return new RequestHeader(
        method, uri, protocol, acceptedResponseProtocols, principal, headers, lowercaseHeaders);
  }
   ...
   *
   * <p>This is a convenience supplied so that server implementations of service calls can pass this
   * request to the request header handler, in order to get the actual incoming request header.
   *
   * <p>See {@link com.lightbend.lagom.javadsl.api.ServiceCall#handleRequestHeader(Function)}
   */
