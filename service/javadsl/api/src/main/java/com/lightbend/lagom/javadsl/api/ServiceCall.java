 ...
 *
 * <p>A service call has a request and a response entity. Either entity may be NotUsed, if there is
 * no entity associated with the call. They may also be an Akka streams Source, in situations where
 * the endpoint serves a stream. In all other cases, the entities will be considered "strict"
 * entities, that is, they will be parsed into memory, eg, using json.
 */
@FunctionalInterface
public interface ServiceCall<Request, Response> {
   ...
   *
   * <p>This should only be used when the request message is NotUsed.
   *
   * @return A future of the response entity.
   */
   ...
   *
   * <p>For client service calls, this gives clients an opportunity to add custom headers and/or
   * modify the request in some way before it is made. The passed in handler is applied before the
   * header transformers configured for the descriptor/endpoint are applied.
   *
   * <p>For server implementations of service calls, this will be invoked by the server in order to
   ...
   */
  default ServiceCall<Request, Response> handleRequestHeader(
      Function<RequestHeader, RequestHeader> handler) {
    // Default implementation. For client service calls, this is overridden by the implementation to
    // do something
    // with the handler.
    return this;
  }
   ...
   * response.
   *
   * <p>For client service calls, this gives clients an opportunity to inspect the response headers
   * and status code. The passed in handler is applied after the header transformers configured for
   * the descriptor/endpoint are applied.
   *
   * <p>For server implementations of service calls, this will be invoked by the server in order to
   ...
   */
  default <T> ServiceCall<Request, T> handleResponseHeader(
      BiFunction<ResponseHeader, Response, T> handler) {
    // Default implementation. For client service calls, this is overridden by the implementation to
    // do something
    // with the handler.
    return request ->
        invoke(request).thenApply(response -> handler.apply(ResponseHeader.OK, response));
  }

  /**
   * Allow handling of the response header.
   *
   * <p>This converts the service call to one that returns both the response header and the response
   * message.
   *
   * <p>This is simply a convenience method for invoking <code>handleResponseHeader(Pair::create)
   * </code>.
   *
