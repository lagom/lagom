...
/**
 * Circuit breaker descriptor. Describes how circuit breaking should be applied to a particular
 * {@link ServiceCall}. A circuit breaker is used to provide stability and prevent cascading
 * failures in distributed systems. An example would be to begin to fail-fast after some number of
 * failures due to a dependency on a hanging call.
 */
public abstract class CircuitBreaker {
   ...
   *
   * <p>Service locators that support per node circuit breaking will typically also remove the node
   * from their routing pool while the circuit breaker is open, and bring it in for one call when
   * it's half open.
   *
   * <p>If not supported by the service locator, a circuit breaker per service may be used.
   */
  ...
  public static final class NamedCircuitBreaker extends CircuitBreaker {
    ...
    // These are defined here, and not on CircuitBreaker, due to classloading deadlocks inherent
    // with super classes
    ...
    public String toString() {
      return "NamedCircuitBreaker{" + "name='" + name + '\'' + '}';
    }
  }
