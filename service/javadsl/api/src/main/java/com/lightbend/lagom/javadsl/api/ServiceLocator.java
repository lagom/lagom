 ...
 * Locates services.
 *
 * <p>The service locator is responsible for two things, one is locating services according to the
 * passed in name and service call information, the other is to implement circuit breaking
 * functionality when {@link #doWithService(String, Descriptor.Call, Function)} is invoked.
 *
 * <p>The reason circuit breaking is a service locator concern is that generally, the service
...
public interface ServiceLocator {
   ...
   *
   * <p>This should be used in preference to {@link #locate(String, Descriptor.Call)} when possible
   * as it will allow the service locator to add in things like circuit breakers.
   *
   * <p>It is required that the service locator will, based on the service call circuit breaker
   * configuration, wrap the invocation of the passed in block with a circuit breaker.
   *
   * @param name The name of the service.
   * @param serviceCall The service call descriptor that this lookup is for.
   * @param block A block of code that takes the URI for the service, and returns a future of some
   ...
   */
  <T> CompletionStage<Optional<T>> doWithService(
