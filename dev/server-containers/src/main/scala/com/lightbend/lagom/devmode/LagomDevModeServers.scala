    ...
    def start(
      ...
      synchronized {
        if (server != null) {
          ...
          val serverClass = loader.loadClass("com.lightbend.lagom.devmode.internal.cassandra.CassandraLauncher")
          server = serverClass.getDeclaredConstructor().newInstance().asInstanceOf[Server]
  ...
  private[lagom] object KafkaServer extends ServerContainer {
    ...
    def start(
      ...
      val process = LagomProcess.runJava(
        ...
        cp,
        "com.lightbend.lagom.devmode.internal.kafka.KafkaLauncher",
        args
      )
