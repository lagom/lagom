...
class StartServiceLocatorMojo @Inject() (logger: MavenLoggerProxy, facade: MavenFacade,
  ...
  @BeanProperty
  var serviceLocatorHost: String = _
  @BeanProperty
  var serviceLocatorPort: Int = _
  @BeanProperty
  var serviceGatewayHost: String = _
  @BeanProperty
  ...
  override def execute(): Unit = {
    ...
    if (serviceLocatorEnabled) {
      ...
      Servers.ServiceLocator.start(logger, scalaClassLoader, cp.map(_.getFile.toURI.toURL).toArray, serviceLocatorHost,
        serviceLocatorPort, serviceGatewayHost, serviceGatewayPort, scalaUnmanagedServices, serviceGatewayImpl)
    }
  }
}
