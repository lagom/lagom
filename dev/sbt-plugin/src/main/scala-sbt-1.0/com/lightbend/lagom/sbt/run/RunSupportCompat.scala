    ...
    allProblems(incomplete) ++ {
      Incomplete.linearize(incomplete).flatMap(getScopedKey).flatMap { scopedKey =>
        ...
        streams.map { streamsManager =>
          ...
          case (Some(error), maybePosition) => new Problem {
            ...
            def position: Position = new Position {
              ...
              def pointer: Optional[Integer] = maybePosition.map(pos => Optional.ofNullable((pos - 1).asInstanceOf[java.lang.Integer])).getOrElse(Optional.empty[java.lang.Integer])
