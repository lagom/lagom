...
import sbt.{ PathFinder, SourceModificationWatch, WatchState }
...
object Reloader {
  ...
  private def urls(cp: Classpath): Array[URL] = cp.map(_.toURI.toURL).toArray
  ...
  def startDevMode(
     ...
     * 2. delegatingLoader, a special classloader that overrides class loading
     ...
     * 3. applicationLoader, contains the application dependencies. Has the
     *    delegatingLoader as its parent. Classes from the commonLoader and
     *    the delegatingLoader are checked for loading first.
     * 4. decoratedClassloader, allows the classloader to be decorated.
    ...
    new DevServer {
      ...
      def addChangeListener(f: Unit => Unit): Unit = reloader.addChangeListener(f)
...
import com.lightbend.lagom.sbt.run.Reloader.{ CompileFailure, CompileResult, CompileSuccess, SourceMap }
...
class Reloader(
  ...
  private val listeners = new java.util.concurrent.CopyOnWriteArrayList[Unit => Unit]()
  ...
  def addChangeListener(f: Unit => Unit): Unit = listeners.add(f)
