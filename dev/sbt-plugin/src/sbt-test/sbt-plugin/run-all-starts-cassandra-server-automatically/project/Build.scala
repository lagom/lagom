...
object DevModeBuild {
  ...
  def getHello(): String = makeRequest("http://localhost:9000/hello/test") { conn =>
  ...
  def isHelloServiceRegistered(): Boolean = makeRequest("http://localhost:8000/services/%2Fhelloservice") { conn =>
    val br = new BufferedReader(new InputStreamReader((conn.getInputStream())))
