...
object DevModeBuild {
  ...
  def isServiceGatewayReachable(): Boolean = makeRequest("http://localhost:9000") { conn =>
    true
  }

  def canAccessFooServiceThroughServiceGateway(): Boolean = makeRequest("http://localhost:9000/foo") { conn =>
    val br = new BufferedReader(new InputStreamReader((conn.getInputStream())))
    val body = Stream.continually(br.readLine()).takeWhile(_ != null).mkString("\n").trim()
    body == "Hello world!"
  }
