...
object ServiceDetector {
  ...
  private val log = Logger(ServiceDetector.getClass)
   ...
   * of all [[com.lightbend.lagom.javadsl.api.Service]]s
   *
   *
   ...
   * @return a JSON array of [[com.lightbend.lagom.internal.api.tools.ServiceDescription]] objects.
   */
  def services(classLoader: ClassLoader): String = {
    ...
    val serviceDescriptions = resolveServiceInterfaces(modules).flatMap { serviceInterface =>
      ...
      if (resolvedDescriptor.locatableService)
        Some(ServiceDescription(resolvedDescriptor))
      else
        None
    }
    Json.stringify(Json.toJson(serviceDescriptions))
  }
  ...
  private[tools] def serviceInterfaceResolver(clazz: Class[_]): Option[Class[_ <: Service]] = {
