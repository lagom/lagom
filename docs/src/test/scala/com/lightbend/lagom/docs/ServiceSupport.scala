...
trait ServiceSupport extends WordSpecLike with Matchers {
  ...
  def withServer[T](applicationBuilder: GuiceApplicationBuilder = new GuiceApplicationBuilder())(block: Application => T): T = {
    ...
    val application =
      applicationBuilder
        .loadConfig { env =>
          Configuration.load(env, Map("lagom.cluster.bootstrap.enabled" -> "off"))
        }
  ...
  def withService[S <: Service : ClassTag, I <: S : ClassTag](applicationBuilder: GuiceApplicationBuilder = new GuiceApplicationBuilder()): WithService[S] =
  ...
  def withServiceInstance[S <: Service : ClassTag](impl: S, applicationBuilder: GuiceApplicationBuilder = new GuiceApplicationBuilder()): WithService[S] =
