...
* Deploy on your platform of choice. To simplify deployment, Lagom supports [the Lightbend Enterprise Suite](https://www.lightbend.com/platform/production) out-of-the-box. Enterprise Suite provides a simple way to deploy, scale, monitor, and manage Lagom services in a container environment. You can run the deployment and monitoring capabilitites of Enterprise Suite on up to three nodes by simply registering with Lightbend. Obtain a [Subscription](https://www.lightbend.com/platform/subscription) to receive world class support and the ability to run more nodes.
