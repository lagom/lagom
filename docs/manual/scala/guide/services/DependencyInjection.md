...
A third method, `describeService`, is optional, but may be used by tooling, for example by [Lightbend Orchestration](https://developer.lightbend.com/docs/lightbend-orchestration/current/features.html#endpoint-detection-declaration), to discover what service APIs are offered by this service. The metadata read from here may in turn be used to configure service gateways and other components.
