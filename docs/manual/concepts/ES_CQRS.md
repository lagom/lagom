...
A benefit of this pattern is the separation of concerns between the write and the read-side. The write-side can focus on the updating commands and the read-side can be optimized for various queries and reporting jobs. A single conceptual model that tries to encapsulate both read and write operations may do neither well.
