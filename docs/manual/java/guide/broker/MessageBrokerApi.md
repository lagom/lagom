...
There are also methods with `andMetadata` suffix  on [`TopicProducer`](api/index.html?com/lightbend/lagom/javadsl/broker/TopicProducer.html) that provide possibility to create [`Message`](api/index.html?com/lightbend/lagom/javadsl/api/broker/Message.html) with metadata.
