...
private[lagom] object InternalCassandraSession {
...
@Singleton
private[lagom] class InternalCassandraSession(system: ActorSystem, settings: CassandraSettings, executionContext: ExecutionContext)
  extends CoreCassandraSession {
  import InternalCassandraSession.ListenableFutureConverter
  import settings._
  ...
  override def underlying(): Future[Session] = {
    underlyingSession()
  }

  override def executeCreateTable(stmt: String): Future[Done] = {
    for {
      s <- underlyingSession()
  ...
  override def prepare(stmt: String): Future[PreparedStatement] =
    underlyingSession().flatMap { _ =>
      preparedStatements.computeIfAbsent(stmt, computePreparedStatement)
    }

  override def executeWriteBatch(batch: BatchStatement): Future[Done] = {
  ...
  override def executeWrite(stmt: Statement): Future[Done] = {
  ...
  @varargs
  override def executeWrite(stmt: String, bindValues: AnyRef*): Future[Done] = {
  ...
  override def select(stmt: Statement): scaladsl.Source[Row, NotUsed] = {
    if (stmt.getConsistencyLevel == null)
      stmt.setConsistencyLevel(readConsistency)
    scaladsl.Source.fromGraph(new SelectSource(Future.successful(stmt)))
  }

  @varargs
  override def select(stmt: String, bindValues: AnyRef*): scaladsl.Source[Row, NotUsed] = {
    val bound: Future[BoundStatement] = prepare(stmt).map { ps =>
      val bs = if (bindValues.isEmpty) ps.bind()
      else ps.bind(bindValues: _*)
    ...
    scaladsl.Source.fromGraph(new SelectSource(bound))
  }

  override def selectAll(stmt: Statement): Future[immutable.Seq[Row]] = {
    if (stmt.getConsistencyLevel == null)
      stmt.setConsistencyLevel(readConsistency)
    scaladsl.Source.fromGraph(new SelectSource(Future.successful(stmt)))
      .runWith(scaladsl.Sink.seq)
  }

  @varargs
  override def selectAll(stmt: String, bindValues: AnyRef*): Future[immutable.Seq[Row]] = {
    val bound: Future[BoundStatement] = prepare(stmt).map(ps =>
  ...
  override def selectOne(stmt: Statement): Future[Option[Row]] = {
    if (stmt.getConsistencyLevel == null)
      stmt.setConsistencyLevel(readConsistency)
    scaladsl.Source.fromGraph(new SelectSource(Future.successful(stmt)))
      .runWith(scaladsl.Sink.headOption)
  }

  @varargs
  override def selectOne(stmt: String, bindValues: AnyRef*): Future[Option[Row]] = {
    val bound: Future[BoundStatement] = prepare(stmt).map(ps =>
      if (bindValues.isEmpty) ps.bind()
      else ps.bind(bindValues: _*))
