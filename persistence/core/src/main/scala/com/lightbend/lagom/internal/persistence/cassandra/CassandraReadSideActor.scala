...
private[lagom] object CassandraReadSideActor {
  ...
  def props[E <: AggregateEvent[E]](tag: String, session: CoreCassandraSession, processorFactory: () => CoreCassandraReadSideProcessor[E]): Props =
  ...
  private def runStream[E <: AggregateEvent[E]](
    ...
    processor: CoreCassandraReadSideProcessor[E],
    session:   CoreCassandraSession,
    log:       LoggingAdapter
    ...
    boundStmts.mapAsync(parallelism = 1) { stmts =>
      stmts.size match {
        case 0 => succ
        case 1 => session.executeWrite(stmts.get(0))
        case _ =>
...
private[lagom] class CassandraReadSideActor[E <: AggregateEvent[E]](
  tag: String, session: CoreCassandraSession, processor: CoreCassandraReadSideProcessor[E]
)
