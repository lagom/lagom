 ...
 */
package com.lightbend.lagom.javadsl.internal.persistence.cassandra
 ...
 */
class CassandraSessionDelegate @Inject() (cassandraSession: CoreCassandraSession, ec: ExecutionContext) extends CassandraSession {
  ...
  def underlying(): CompletionStage[Session] =
    cassandraSession.underlying().toJava
  ...
  def executeCreateTable(stmt: String): CompletionStage[Done] =
    cassandraSession.executeCreateTable(stmt).toJava
  ...
  def prepare(stmt: String): CompletionStage[PreparedStatement] =
    cassandraSession.prepare(stmt).toJava
  ...
  def executeWriteBatch(batch: BatchStatement): CompletionStage[Done] =
    cassandraSession.executeWriteBatch(batch).toJava
  ...
  def executeWrite(stmt: Statement): CompletionStage[Done] =
    cassandraSession.executeWrite(stmt).toJava
  ...
  def executeWrite(stmt: String, bindValues: AnyRef*): CompletionStage[Done] =
    cassandraSession.executeWrite(stmt, bindValues).toJava
  ...
  def select(stmt: Statement): javadsl.Source[Row, NotUsed] =
    cassandraSession.select(stmt).asJava
  ...
  def select(stmt: String, bindValues: AnyRef*): javadsl.Source[Row, NotUsed] =
    cassandraSession.select(stmt, bindValues).asJava
   ...
   */
  def selectAll(stmt: Statement): CompletionStage[PSequence[Row]] =
    cassandraSession.selectAll(stmt).map(c => TreePVector.from(c.asJava).asInstanceOf[PSequence[Row]])(ec).toJava
   ...
   */
  @varargs
  def selectAll(stmt: String, bindValues: AnyRef*): CompletionStage[PSequence[Row]] =
    cassandraSession.selectAll(stmt, bindValues).map(c => TreePVector.from(c.asJava).asInstanceOf[PSequence[Row]])(ec).toJava
  ...
  def selectOne(stmt: Statement): CompletionStage[Optional[Row]] =
    cassandraSession.selectOne(stmt).map(c => c.asJava)(ec).toJava
  ...
  def selectOne(stmt: String, bindValues: AnyRef*): CompletionStage[Optional[Row]] =
    cassandraSession.selectOne(stmt, bindValues).map(c => c.asJava)(ec).toJava
