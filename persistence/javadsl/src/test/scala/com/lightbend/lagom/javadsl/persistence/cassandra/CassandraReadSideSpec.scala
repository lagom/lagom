...
object CassandraReadSideSpec {
  ...
  class TestEntityProcessor(implicit ec: ExecutionContext) extends CoreCassandraReadSideProcessor[TestEntity.Evt] {
    ...
    override def prepare(session: CoreCassandraSession): CompletionStage[Optional[UUID]] = {
      def prepareWriteStmt(): Future[Unit] =
        session.prepare("INSERT INTO testcounts (id, count, offset) VALUES (?, ?, ?)")
          .map(writeStmt = _)

      def selectCountAndOffset(): Future[Optional[UUID]] = {
        for {
          selectStmt <- session.prepare("SELECT count, offset FROM testcounts WHERE id = ?")
          bs = selectStmt.bind("test")
          row <- session.selectOne(bs)
        } yield {
          if (row.isDefined) {
...
class CassandraReadSideSpec extends PersistenceSpec(CassandraReadSideSpec.config) {
  ...
  lazy val testSession: CoreCassandraSession = new InternalCassandraSession(system)
  ...
  lazy val selectStmt: PreparedStatement =
    Await.result(testSession.prepare("SELECT count FROM testcounts WHERE id = ?"), 5.seconds)

  def assertSelectCount(expected: Long): Unit = {
    within(20.seconds) {
      awaitAssert {
        val count = Await.result(testSession.selectOne(selectStmt.bind("test")).map(_.get.getLong("count")), 5.seconds)
