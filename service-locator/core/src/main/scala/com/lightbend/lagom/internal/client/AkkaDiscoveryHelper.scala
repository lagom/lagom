...
/**
  ...
  */
private[lagom] class AkkaDiscoveryHelper(config: Config, serviceDiscovery: SimpleServiceDiscovery)(implicit ec: ExecutionContext) {
  ...
  private val portNameSchemeMapping = {
    ...
    config.getObject("port-name-scheme-mapping").asScala
      .map {
  ...
  def locateAll(name: String): Future[Seq[URI]] = {
    ...
    serviceDiscovery.lookup(lookupQuery, lookupTimeout)
      .map { resolved =>
  ...
  private def toURI(resolvedTarget: ResolvedTarget, lookup: Lookup): URI = {
    ...
    val scheme = lookup.portName.flatMap(portNameSchemeMapping.get)
