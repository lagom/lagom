...
def mimaSettings(since: String = version140): Seq[Setting[_]] = {
...
lazy val `akka-discovery-service-locator-core` = (project in file("akka-service-locator/core"))
  .settings(runtimeLibCommon: _*)
  .settings(mimaSettings(since = version1412): _*)
  .enablePlugins(RuntimeLibPlugins)
  .settings(
...
lazy val `build-tool-support` = (project in file("dev") / "build-tool-support")
  ...
  .settings(
    ...
    mimaBinaryIssueFilters ++= Seq(
      // Project2PortMapBuilder is a private extension class to private[lagom] PortRange
      ProblemFilters.exclude[DirectMissingMethodProblem]("*lagom.dev.PortAssigner#Project2PortMapBuilder*")
    )
  )
              ...
              Def.setting {
                ...
                Dependencies.ScalaVersions.map {
                  supportedVersion =>
                    ...
                    val crossFunc =
                      CrossVersion(
                        new Binary(binaryScalaVersion),
                        supportedVersion,
                        binaryScalaVersion(supportedVersion)
                      ).get
