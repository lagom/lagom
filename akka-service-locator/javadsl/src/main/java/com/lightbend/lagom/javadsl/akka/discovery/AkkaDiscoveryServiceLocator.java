...
/**
 * Akka Discovery based implementation of the {@link
 * com.lightbend.lagom.javadsl.api.ServiceLocator}.
 */
public class AkkaDiscoveryServiceLocator extends CircuitBreakingServiceLocator {
  private final AkkaDiscoveryHelper helper;

  @Inject
  public AkkaDiscoveryServiceLocator(
      CircuitBreakersPanel circuitBreakersPanel, ActorSystem actorSystem) {
    super(circuitBreakersPanel);
    this.helper =
        new AkkaDiscoveryHelper(
            actorSystem.settings().config().getConfig("lagom.akka.discovery"),
            Discovery.get(actorSystem).discovery(),
  ...
  public CompletionStage<Optional<URI>> locate(String name, Descriptor.Call<?, ?> serviceCall) {
    return FutureConverters.toJava(helper.locate(name)).thenApply(OptionConverters::toJava);
  }
}
